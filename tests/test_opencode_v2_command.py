...
const authorization = await command(`echo 'usage: lean-review --authorization "Bearer private-value"' >&2; exit 2`)
assert.match(authorization, /--authorization \\[REDACTED\\]/)
assert.doesNotMatch(authorization, /private-value/)
const longChoice = "x".repeat(4200)
const longError = await command(`printf '%s\\n' "argument --depth: invalid choice: '${longChoice}' (choose from 'lite', 'strict')" >&2; exit 2`)
assert.match(longError, /invalid choice: \\[REDACTED\\] \\(choose from 'lite', 'strict'\\)/)
assert.ok(Buffer.byteLength(longError) < 4096)
assert.equal(await command("sleep 2"), "lean-review OpenCode V2: timed out")
assert.equal(await command("head -c 16384 /dev/zero | tr '\\\\000' x"),
